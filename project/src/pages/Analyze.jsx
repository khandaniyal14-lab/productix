...
import { BarChart3, TrendingUp, AlertTriangle, Lightbulb, AlertCircle, Plus, Trash2, Target, Gauge, ArrowRight } from 'lucide-react';
import { analysisService } from '../services/api';

const inputOptions = [
  { value: "Direct Labor Hours", label: "Direct Labor Hours" },
  { value: "Electricity Consumption", label: "Electricity Consumption" },
  { value: "Machine Hours", label: "Machine Hours" },
  { value: "Raw Material Usage", label: "Raw Material Usage" },
...
const Analyze = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Convert inputs array to single object
      ...
      // Prepare single_productivity object
      ...
      Object.entries(formattedInputs).forEach(([inputKey, inputVal]) => {
        Object.entries(formattedOutputs).forEach(([outputKey, outputVal]) => {
          singleProductivityObj[`${inputKey} / ${outputKey}`] =
            ((inputVal / outputVal) * 100).toFixed(2);
        });
      });

      // Build payload according to backend schema
      const payload = {
        combined_productivity: { value: Number(combinedProductivity) },
        single_productivity: singleProductivityObj,
        targeted_productivity: { value: Number(targetedProductivity) },
        standard_productivity: { value: Number(standardProductivity) },
        inputs: formattedInputs,       // now a dictionary
        outputs: formattedOutputs,     // now a dictionary
      };
      ...
      if (err.response?.data?.detail) {
        setError(
          err.response.data.detail.map(d => {
            const loc = Array.isArray(d.loc) ? d.loc.join(" > ") : d.loc;
            const input = typeof d.input === "object" ? JSON.stringify(d.input) : d.input;
            return `${loc}: ${d.msg} (input: ${input})`;
          })
        );
      } else {
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 p-6 rounded-lg">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-8">
          ...
          <p className="text-white/40 text-sm">Get AI-powered insights and recommendations for your production efficiency</p>
        </div>
        ...
        <div className="space-y-6 mb-8">
          ...
          <div className="bg-gradient-to-br from-white/5 to-white/[0.02] backdrop-blur-xl rounded-2xl border border-white/10 p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              <div>
                <label className="block text-white/60 text-sm font-medium mb-2">Combined Productivity (%)</label>
                <input
                  type="number"
                  value={combinedProductivity}
                  onChange={(e) => setCombinedProductivity(e.target.value)}
                  className="w-full bg-white/10 border border-white/10 rounded-xl py-3 px-4 text-white focus:outline-none focus:border-purple-500/50 focus:bg-white/15 transition-all duration-300"
                  placeholder="85.5"
                  step="0.01"
                />
              </div>
              ...
              <div>
                <label className="block text-white/60 text-sm font-medium mb-2">Single Productivity (%)</label>
                <input
                  type="number"
              ...
              <div>
                <label className="block text-white/60 text-sm font-medium mb-2">Targeted Productivity (%)</label>
                <input
                  type="number"
              ...
              <div>
                <label className="block text-white/60 text-sm font-medium mb-2">Standard Productivity (%)</label>
                <input
                  type="number"
              ...
              <div>
                <div className="flex items-center justify-between mb-4">
                  ...
                  <button
                    type="button"
                    onClick={addInput}
                    className="p-2 bg-purple-500/10 hover:bg-purple-500/20 border border-purple-500/20 hover:border-purple-500/40 rounded-lg transition-all duration-300 hover:scale-110"
                ...
                <div className="space-y-3 max-h-[400px] overflow-y-auto pr-2 custom-scrollbar">
                  {inputs.map((input, index) => (
                    <div key={index} className="bg-white/5 rounded-xl p-3 border border-white/10 hover:border-purple-500/30 transition-all duration-300">
                      <Select
                      ...
                      <div className="flex items-center gap-2 mt-2">
                        <input
                          ...
                          className="flex-1 bg-white/10 border border-white/10 rounded-lg py-2.5 px-3 text-white text-sm focus:outline-none focus:border-purple-500/50 transition-all duration-300"
                          placeholder="Value"
                        />
                        {inputs.length > 1 && (
                          <button
              ...
              <div>
                ...
                <div className="space-y-3 max-h-[400px] overflow-y-auto pr-2 custom-scrollbar">
                  {outputs.map((output, index) => (
                    <div key={index} className="bg-white/5 rounded-xl p-3 border border-white/10 hover:border-emerald-500/30 transition-all duration-300">
                      <Select
                      ...
                      <div className="flex items-center gap-2 mt-2">
                        <input
                          ...
                          className="flex-1 bg-white/10 border border-white/10 rounded-lg py-2.5 px-3 text-white text-sm focus:outline-none focus:border-emerald-500/50 transition-all duration-300"
                          placeholder="Value"
                        />
                        {outputs.length > 1 && (
                          <button
        ...
        <div className="bg-gradient-to-br from-white/5 to-white/[0.02] backdrop-blur-xl rounded-2xl border border-white/10 p-6">
          ...
          {results ? (
            <div className="space-y-4">
              {/* Efficiency Score - Large Featured Card */}
              <div className="bg-gradient-to-br from-green-500/10 to-emerald-500/10 border border-green-500/20 rounded-xl p-6 hover:border-green-500/40 transition-all duration-300">
                ...
                <p className="text-green-400/70 text-sm">Above target performance</p>
              </div>
            ...
            <div className="text-center py-16">
              ...
              <p className="text-white/40">Complete the form and click analyze to see AI-powered insights</p>
            </div>
          )}
        </div>
      </div>
